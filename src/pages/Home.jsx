...
const icons = {
  ...
  brain:     "M9.5 2a2.5 2.5 0 015 0M12 9v3m0 0v3m0-3h3m-3 0H9M5.5 7.5A4 4 0 014 11a8 8 0 008 8 8 8 0 008-8 4 4 0 00-1.5-3.5",
      ...
      <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center gap-12">
        ...
        <div className="flex-1 max-w-xl">
          ...
          <p className="text-slate-500 text-base leading-relaxed mb-8">
            ...
            smarter resolution. Join thousands of citizens making their neighborhood better today.
          </p>
          ...
          <div className="relative w-full max-w-md">
            ...
            <div className="relative bg-white border border-slate-100 rounded-3xl shadow-xl p-6 overflow-hidden">
              ...
              <div className="flex items-end gap-2 h-28 mb-4">
                {[60,80,45,95,70,55,85,65,90,50,75,88].map((h, i) => (
              ...
              <div className="grid grid-cols-3 gap-2">
                {[["12k+","Resolved"],["50k","Users"],["85","Cities"]].map(([n,l]) => (
                  <div key={l} className="bg-blue-50 rounded-xl p-2 text-center">
...
// ─── Features ─────────────────────────────────────────────────────────────────
const features = [
  { icon: icons.brain,     title: "AI Issue Detection",   desc: "Automated classification of reports using computer vision to prioritize critical safety concerns instantly." },
  { icon: icons.activity,  title: "Real-Time Tracking",   desc: "Follow your report from start to finish with live status updates and direct notifications on progress." },
  { icon: icons.map,       title: "Heatmap Analytics",    desc: "Visualize city hotspots and recurring trends to allocate municipal resources where they are needed most." },
  { icon: icons.bar,       title: "Smart Dashboard",      desc: "Data-driven insights for authorities to monitor city-wide health and team response performance." },
  { icon: icons.clipboard, title: "Complaint Management", desc: "Streamlined workflow for city staff to assign, manage, and close tickets without the bureaucratic noise." },
  { icon: icons.monitor,   title: "Social Monitoring",    desc: "Listening to civic concerns everywhere by aggregating reports from public social media mentions." },
]
    ...
    <section className="py-20 bg-white">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-14">
          <h2 className="text-3xl md:text-4xl font-extrabold text-slate-800 mb-3">Comprehensive Urban Management</h2>
          <p className="text-slate-400 max-w-md mx-auto text-sm leading-relaxed">
            Everything municipal teams and citizens need to collaborate on a cleaner, safer city environment.
          </p>
        </div>
...
// ─── How It Works ─────────────────────────────────────────────────────────────
const steps = [
  { icon: icons.alert,   label: "Report Issue",        desc: "Snap a photo and pin the location in seconds." },
  { icon: icons.brain,   label: "AI Analysis",         desc: "Our AI categorizes and validates your report instantly." },
  { icon: icons.monitor, label: "Authority Dashboard", desc: "Local authorities receive and triage the task." },
  { icon: icons.check,   label: "Resolution",          desc: "The issue is fixed and you're notified of completion." },
]
  ...
  return (
    <section className="py-20 bg-slate-50">
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-14">
          <h2 className="text-3xl md:text-4xl font-extrabold text-slate-800 mb-3">How It Works</h2>
          <p className="text-slate-400 text-sm">Simple steps toward a better city.</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="text-slate-400 text-xs leading-relaxed">
              Empowering smarter cities through citizen-led resolution. One report at a time.
            </p>
          </div>

          {/* Quick Links */}
          <div>
            <div className="font-semibold text-slate-700 text-sm mb-4">Quick Links</div>
            {["Features","Success Stories","Pricing","Support"].map(l => (
              <a key={l} href="#" className="block text-slate-400 hover:text-blue-600 text-sm mb-2 transition-colors">{l}</a>
            ))}
          </div>

          {/* Legal */}
          <div>
            <div className="font-semibold text-slate-700 text-sm mb-4">Legal</div>
          ...
          <div className="hidden md:block">
            <div className="font-semibold text-slate-700 text-sm mb-4">Social Media</div>
            <div className="flex gap-3 text-slate-400">
              {[icons.twitter, icons.instagram, icons.share].map((d, i) => (
                <button key={i} className="w-8 h-8 rounded-full border border-slate-200 hover:border-blue-300 hover:text-blue-500 flex items-center justify-center transition-colors">
                  <Icon d={d} size={14} />
                </button>
              ))}
            </div>
