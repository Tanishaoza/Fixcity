...
const createPinIcon = (color, pulseColor) =>
  L.divIcon({
    className: "",
    iconAnchor: [12, 30],
    popupAnchor: [0, -30],
...
function StatCard({ label, value, icon: Icon, accent, total }) {
  ...
  return (
    <div
      className={`relative rounded-2xl border p-5 bg-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-default overflow-hidden ${t.soft}`}
    >
      <div className="flex items-start justify-between mb-4">
        <div
          className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${t.bg} flex items-center justify-center shadow-lg ${t.shadow}`}
        >
      ...
      <div
        className={`text-4xl font-black tracking-tight leading-none mb-1 ${t.text}`}
      >
        {value}
      </div>
      <p
        className={`text-[11px] font-extrabold uppercase tracking-[0.12em] mb-3 ${t.sub}`}
      >
        {label}
      </p>
    ...
    <>
      ...
      >
        <span className="flex items-center gap-1.5">
          <span
            className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[currentStatus] || "bg-slate-400"}`}
          />
          {currentStatus}
        </span>
        {updating ? (
          ...
          >
            {statuses.map((s) => (
              <button
                ...
                onClick={() => {
                  setOpen(false);
                  if (s !== currentStatus) onUpdate(issueId, s);
                }}
              ...
              >
                <span
                  className={`w-2 h-2 rounded-full flex-shrink-0 ${STATUS_DOT[s] || "bg-slate-300"}`}
                />
                <span
                  className={
                    s === currentStatus ? "text-blue-700" : "text-slate-700"
                  }
                >
                  {s}
                </span>
                {s === currentStatus && (
                  <CheckCircle2 size={11} className="ml-auto text-blue-500" />
                )}
              </button>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-3.5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-[11px] leading-none mt-0.5">
                FixCity · Municipal Civic Issue Management System
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex gap-3 flex-wrap">
              {[
                ...
                <div
                  key={label}
                  className="bg-white/10 backdrop-blur rounded-xl px-4 py-2.5 text-center border border-white/20"
                >
        ...
        <div className="bg-white/80 backdrop-blur rounded-2xl border border-slate-200/60 shadow-sm p-4">
          <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
            <div className="relative flex-1 group">
              ...
              <input
                type="text"
                placeholder="Search by ID, title, reporter, category or location…"
                value={search}
            ...
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-500 whitespace-nowrap">
              <TrendingUp size={13} /> {filtered.length} of {issues.length}{" "}
              issues
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2 flex-wrap">
                {["Pending", "In Review", "Assigned", "Resolved"].map((s) => (
                  <div
                    key={s}
                    className={`hidden sm:flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full ${STATUS_BADGE[s]}`}
                  >
                    <div
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-start gap-3">
                            <div
                              className={`mt-1 w-1.5 h-8 rounded-full flex-shrink-0 ${SEVERITY_BAR[issue.severity] || "bg-slate-200"}`}
                            />
                            <div>
                              <span className="text-[10px] font-bold text-slate-400 font-mono block mb-0.5">
                                {issue.issueId}
                              </span>
                              <span className="font-bold text-slate-800 text-sm leading-snug group-hover:text-blue-700 transition-colors block">
                                {issue.title}
                              </span>
                        ...
                        <td className="px-5 py-4">
                          {issue.image ? (
                            ...
                            <div className="w-14 h-14 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center">
                              <span className="text-[9px] text-slate-400 font-semibold text-center leading-tight px-1">
                                No Image
                              </span>
                            </div>
                          )}
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full ${PRIORITY_BADGE[issue.priority] || ""}`}
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${PRIORITY_DOT[issue.priority] || "bg-slate-400"}`}
                            />
                            {issue.priority}
                          </span>
                        </td>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  Areas with more reports glow brighter on the map.
                </p>
              </div>
              ...
              {topAreas.length === 0 ? (
                ...
                topAreas.map(([area, count], i) => {
                  ...
                  return (
                    <div
                      key={area}
                      className={`flex items-center gap-3 p-3.5 rounded-xl border ${bgs[i] || "bg-slate-50 border-slate-200"}`}
                    >
                      <div
                        className={`w-9 h-9 rounded-xl bg-gradient-to-br ${colors[i] || "from-slate-400 to-slate-600"} flex items-center justify-center text-white font-black text-sm flex-shrink-0`}
                      >
                        #{i + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p
                          className={`font-bold text-xs leading-snug line-clamp-1 ${texts[i] || "text-slate-700"}`}
                        >
                          {area}
                        </p>
                      ...
                      <div
                        className={`text-lg font-black ${texts[i] || "text-slate-500"}`}
                      >
                        {count}
                      </div>
                    </div>
