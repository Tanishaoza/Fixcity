...
const PRIORITY_CONFIG = {
  Critical: { bg: "bg-red-50",     text: "text-red-700",     border: "border-red-200",     dot: "bg-red-500",     bar: "bg-red-500",     icon: "🔴" },
  Moderate: { bg: "bg-amber-50",   text: "text-amber-700",   border: "border-amber-200",   dot: "bg-amber-500",   bar: "bg-amber-500",   icon: "🟡" },
  Minor:    { bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-200", dot: "bg-emerald-500", bar: "bg-emerald-500", icon: "🟢" },
};
...
function HeatLayer({ issues }) {
  ...
  useEffect(() => {
    const pts = issues
      ...
      .map((i) => [+i.latitude, +i.longitude, 0.7]);
...
function Badge({ children, className = "" }) {
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border ${className}`}>
      {children}
    </span>
  );
}
...
function StatCard({ label, value, icon: Icon, grad, soft, val, bar, total, isTotal }) {
  ...
  return (
    <div className={`group relative rounded-2xl border p-4 bg-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 overflow-hidden ${soft}`}>
      <div className="flex items-start justify-between mb-3">
        ...
        {!isTotal && total > 0 && (
          <span className={`text-xs font-black ${val}`}>{pct}%</span>
        )}
      </div>
    ...
    <>
      ...
      >
        <span className="flex items-center gap-1.5">
          <Dot className={cfg.dot || "bg-slate-300"} />
          {currentStatus}
        </span>
        {updating
      ...
      {open && (
        <>
          ...
          <div
            className="fixed z-[9999] bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden py-1"
    ...
    <tr className="hover:bg-slate-50/60 transition-colors group border-b border-slate-100 last:border-0">
      ...
      <td className="px-4 py-3.5">
        <Badge className={`${pri.bg || "bg-slate-50"} ${pri.text || "text-slate-600"} ${pri.border || "border-slate-200"}`}>
          <Dot className={pri.dot || "bg-slate-400"} />
          {issue.priority || "—"}
        </Badge>
      </td>
      ...
      <td className="px-4 py-3.5 max-w-[160px]">
        <div className="flex flex-col gap-1">
          <Badge className={`${aiCfg.bg} ${aiCfg.text} border-transparent w-fit`}>
...
export default function AdminDashboard() {
  ...
  const filtered = issues.filter((issue) => {
    ...
    const matchSearch = !q || [
      ...
      issue.location, issue.name, issue.email,
        ...
        <section>
          ...
          {loading ? (
            ...
            <div className="bg-red-50 rounded-xl border border-red-200 py-16 flex flex-col items-center gap-3">
              ...
              <p className="text-red-400 text-xs">Server may be starting up (Render cold start). Try refreshing in 30s.</p>
              <button
                onClick={fetchIssues}
                className="mt-1 text-xs text-red-600 font-bold border border-red-200 px-4 py-2 rounded-lg hover:bg-red-100 transition-colors"
              >
                Try Again
              </button>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 py-20 flex flex-col items-center gap-3 shadow-sm">
              ...
              <p className="text-slate-400 text-xs">
                {search || statusFilter !== "All" ? "Try adjusting your search or filter." : "No civic issues reported yet."}
              </p>
              {(search || statusFilter !== "All") && (
                <button
                  onClick={() => { setSearch(""); setStatus("All"); }}
                  className="text-xs text-blue-600 font-bold border border-blue-200 px-4 py-2 rounded-lg hover:bg-blue-50 transition-colors"
                >
                  Clear filters
                </button>
              )}
                  ...
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      {["Issue", "Location", "Image", "Reporter", "Priority", "AI Analysis", "Date", "Status"].map((h) => (
                        <th key={h} className="px-4 py-3 text-left text-[10px] font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap">
                          {h}
                        </th>
                      ))}
                    </tr>
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  <MapPin size={14} className="text-blue-500" /> Issue Heatmap
                </h3>
                      ...
                      <Popup>
                        <div className="min-w-[160px] font-sans">
                          ...
                          <p className="text-xs mb-0.5">
                            Priority: <strong style={{ color: issue.priority === "Critical" ? "#dc2626" : issue.priority === "Moderate" ? "#d97706" : "#16a34a" }}>{issue.priority}</strong>
                          </p>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex flex-col gap-4">
              ...
              {topAreas.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {topAreas.map(([area, count], i) => {
                    ...
                    const tag = count >= 5 ? { label: "⚠ Major zone", cls: "text-red-600" } :
                                count >= 2 ? { label: "Monitor closely", cls: "text-amber-600" } :
                                             { label: "Normal activity", cls: "text-slate-400" };
                    return (
                      <div key={area} className={`flex items-center gap-2.5 p-3 rounded-lg border ${c.bg} ${c.border}`}>
                        <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${c.num} flex items-center justify-center text-white font-black text-xs flex-shrink-0`}>
                          #{i + 1}
                        </div>
            ...
            <div className="border-t border-slate-100 pt-3 mt-auto">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-[11px] font-black text-emerald-600">{resolutionRate}%</span>
              </div>
